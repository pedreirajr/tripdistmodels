...
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE.md) [![Lifecycle: experimental](https://img.shields.io/badge/lifecycle-experimental-orange.svg)](https://lifecycle.r-lib.org/articles/stages.html#experimental)

Current scope:

-   **Growth factoring with Iterative Proportional Fitting (Furness)** to update a base OD to future productions and attractions.
...
-   Power: `c^(-n)` with `n > 0`
-   Combined: `c^(n) * exp(-beta * c)` with `beta > 0` and `n` real (may be negative)
...
`syn_gravitational()` returns an object of class `"tripdistmodels_syn_gravity"` with:
